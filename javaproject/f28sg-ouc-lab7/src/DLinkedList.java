...
public class DLinkedList {
	 ...
	 * O(N^2)
	 * 
	 * Because: for each iteration I need to travel through the whole linkedlist,
	 * and to get the right position, I need to go through the previous nodes to
	 * compare with the currentNode
	 */
	public void insertionSort() {
		if (headNode == null || headNode.getNextNode() == null) {
			return;
		}
		//I need  a temporary pointer to go through the whole linked list
		...
		while (currentNode != null) {
			...
			//delete the currentNode to find a right position for it
			delete(currentNode);
			//this is not the right position so move the prevNode again
			while (prevNode != null && prevNode.getValue() > currentValue) {
				prevNode = prevNode.getPrevNode();
			}

			if (prevNode == null) {

				addAtHead(currentValue);
			} else {
				insertAfter(prevNode, currentValue);
			}
			//move the right now pointer to do this comparison again and again
